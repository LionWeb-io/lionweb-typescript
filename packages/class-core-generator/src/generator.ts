...
import {
    ...
    displayDirectSpecializationsPerClassifierOnConsole
...
export const generateLanguage = (language: Language, generationPath: string, lionWebVersion: LionWebVersion, mayBeOptions?: Partial<GeneratorOptions>): string => {
    ...
    const specializationMap = directSpecializationsPerClassifier([language])
    displayDirectSpecializationsPerClassifierOnConsole(specializationMap)
    writeFileSync(join(generationPath, fileName), languageFileFor(language, lionWebVersion, withDefaults(mayBeOptions), specializationMap))
    return fileName
}
