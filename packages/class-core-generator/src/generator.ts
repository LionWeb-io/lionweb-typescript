...
import { directSubsPerClassifier } from "./helpers/index.js"
...
export const generateLanguage = (language: Language, generationPath: string, lionWebVersion: LionWebVersion, mayBeOptions?: Partial<GeneratorOptions>): string => {
    ...
    writeFileSync(join(generationPath, fileName), languageFileFor(language, lionWebVersion, withDefaults(mayBeOptions), directSubsPerClassifier([language])))
    return fileName
}
