...
* **Breaking changes!** Remove _all_ items marked as `[@]deprecated` or as being “legacy” (but not yet `@deprecated`) in their JSDocumentation.
  ...
  * Remove the variants of the `nodeBaseDetailedDeserializer` and `nodeBaseDeserializer` functions that take anything other than one (full) configuration object.
  ...
  This is documented in the following in the form “`<deprecated/legacy item>` &rarr; `<use instead>`”:
    ...
    * `nodeBaseDeserializerWithIdMapping` &rarr; `nodeBaseDetailedDeserializerWith`
