import { deserializeLanguages, serializeLanguages, LionWebVersions } from "@lionweb/core"
...
const checkNode = (node: LionWebJsonNode) => {
    ...
    ids.push(id)
    const key = lookUpPropertyValueByKey(node, v2023_1.lioncoreFacade.metaFeatures.ikeyed_key.key)
    if (key === null) {
    ...
    const name = lookUpPropertyValueByKey(node, v2023_1.builtinsFacade.features.inamed_name.key)
    if (name === null) {
...
const shapesLanguage = deserializeLanguages(shapesJson, v2023_1.builtinsFacade.language)[0]
