...
* `src/`:
  ...
  * `generate-*.ts`: several Node.js “scripts” that are invoked as part of the NPM `generate` task.
    * A specific mention for `generate-for-test-language`: this will try to obtain a `testLanguage.2023.1.json` file from either the [`lionweb-integration-testing` GitHub repository](https://raw.githubusercontent.com/LionWeb-io/lionweb-integration-testing/refs/heads/main/src/languages/testLanguage.2023.1.json),
        ...
        Running this Node.js script with the flag `--force-local` will force it to skip trying to obtain that JSON file from GitHub.
