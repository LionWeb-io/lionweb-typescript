...
* Widen the `abstract` argument to both the `new Concept` (5th argument) and `LanguageFactory.concept` (2nd argument) calls, to include the `ClassModifier` enum with literals `concrete` and `abstract`.
