 ...
 */
export const createBrowserLowLevelClient = async <TMessageForClient, TMessageToServer>(
    ...
    return new Promise((resolveClientStart, rejectClientStart) => {
        ...
        webSocket.addEventListener("message", (event) => {
            const { data } = event  // FIXME  inspect event.type &c.
            const message = tryParseJson(data, (message) => log({ message })) as TMessageForClient
