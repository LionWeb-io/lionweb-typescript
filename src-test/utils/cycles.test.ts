...
import {cycleWith} from "../../src-pkg/index.js"
...
describe("cycleWith", () => {
    ...
    it("should not detect no-cycles", () => {
        deepEqual(cycleWith(0, (x) => x < 4 ? [x + 1] : []), [])
    })

    it("should detect a trivial cycle", () => {
        deepEqual(cycleWith(0, (_) => [0]), [0, 0])
    })

    it("should detect a simple cycle", () => {
        deepEqual(cycleWith(0, (x) => [(x + 1) % 4]), [0, 1, 2, 3, 0])
    })

    it("should detect a more complex cycle", () => {
        //          0      1         2      3      4      5
        const t = [ [1], [2, 3], [4], [5], [0], [] ]
        deepEqual(cycleWith(0, (x) => t[x]), [0, 1, 2, 4, 0])
    })
    ...
    it("inverse Collatz function works", () => {
        for (let n = 1; n < 1000; n++) {
            const inv = inverseCollatz(n)
            isTrue(inv.length > 0)
            inv.forEach((m) => deepEqual(n, collatz(m)))
        }
    })

    it("should detect a cycle in the Collatz graph", () => {
        const collatzAncestors = (n: number) => n < 100 ? inverseCollatz(n) : []
        deepEqual(cycleWith(1, collatzAncestors), [1, 2, 1])
    })
